...
class _InventoryManagementState extends State<InventoryManagement> {
  ...
  Map<String, dynamic> _activeFilters = {
    'stockStatus': null, 'category': null,
    'supplier': null, 'location': null, 'lastUpdated': null,
  };
  ...
  final List<Map<String, dynamic>> _inventoryItems = [
    {"id": 1, "name": "Tomatoes",      "category": "Raw Materials",
     "image": "https://images.unsplash.com/photo-1546094096-0df4bcaaa337?w=400&q=80",
     "currentStock": 45, "minThreshold": 20, "unit": "kg",
     "status": "Adequate",  "supplier": "Supplier A", "location": "Main Store", "lastUpdated": "12/11/2025"},
    {"id": 2, "name": "Onions",        "category": "Raw Materials",
     "image": "https://images.unsplash.com/photo-1587049352846-4a222e784d38?w=400&q=80",
     "currentStock": 15, "minThreshold": 25, "unit": "kg",
     "status": "Low Stock", "supplier": "Supplier A", "location": "Main Store", "lastUpdated": "12/11/2025"},
    {"id": 3, "name": "Chicken Breast","category": "Raw Materials",
     "image": "https://images.unsplash.com/photo-1604503468506-a8da13d82791?w=400&q=80",
     "currentStock": 5,  "minThreshold": 15, "unit": "kg",
     "status": "Critical", "supplier": "Supplier B", "location": "Kitchen",    "lastUpdated": "12/11/2025"},
    {"id": 4, "name": "Potatoes",      "category": "Raw Materials",
     "image": "https://images.unsplash.com/photo-1518977676601-b53f82aba655?w=400&q=80",
     "currentStock": 5,  "minThreshold": 15, "unit": "kg",
     "status": "Low Stock", "supplier": "Supplier A", "location": "Main Store", "lastUpdated": "12/11/2025"},
    {"id": 5, "name": "Olive Oil",     "category": "Store Items",
     "image": "https://images.unsplash.com/photo-1474979266404-7eaacbcd87c5?w=400&q=80",
     "currentStock": 12, "minThreshold": 8,  "unit": "bottles",
     "status": "Adequate",  "supplier": "Supplier C", "location": "Counter",    "lastUpdated": "12/10/2025"},
    {"id": 6, "name": "Pasta",         "category": "Store Items",
     "image": "https://images.unsplash.com/photo-1551462147-ff29053bfc14?w=400&q=80",
     "currentStock": 8,  "minThreshold": 15, "unit": "packets",
     "status": "Low Stock", "supplier": "Supplier C", "location": "Main Store", "lastUpdated": "12/10/2025"},
    {"id": 7, "name": "Coffee Beans",  "category": "Store Items",
     "image": "https://images.unsplash.com/photo-1559056199-641a0ac8b55e?w=400&q=80",
     "currentStock": 3,  "minThreshold": 10, "unit": "kg",
     "status": "Critical", "supplier": "Supplier B", "location": "Counter",    "lastUpdated": "12/09/2025"},
    {"id": 8, "name": "Burger Buns",   "category": "Finished Goods",
     "image": "https://images.unsplash.com/photo-1568901346375-23c9450c58cd?w=400&q=80",
     "currentStock": 50, "minThreshold": 30, "unit": "pieces",
     "status": "Adequate",  "supplier": "In-house",   "location": "Kitchen",    "lastUpdated": "12/11/2025"},
    {"id": 9, "name": "Pizza Dough",   "category": "Finished Goods",
     "image": "https://images.unsplash.com/photo-1513104890138-7c749659a591?w=400&q=80",
     "currentStock": 10, "minThreshold": 20, "unit": "pieces",
     "status": "Low Stock", "supplier": "In-house",   "location": "Kitchen",    "lastUpdated": "12/11/2025"},
  ];

  static const _categories = ['Raw Materials','Store Items','Finished Goods','All Items'];

  // Weekly usage mock data (Mon–Wed repeated)
  static const _weeklyData = [80.0, 140.0, 110.0, 200.0, 160.0, 230.0, 190.0];
  static const _weekDays   = ['Mon','Tue','Wed','Thu','Fri','Sat','Sun'];

  List<Map<String, dynamic>> get _filteredItems => _inventoryItems.where((item) {
    if (_selectedCategory != 'All Items' && item['category'] != _selectedCategory) return false;
    if (_searchController.text.isNotEmpty) {
      if (!(item['name'] as String).toLowerCase().contains(_searchController.text.toLowerCase())) return false;
    }
    if (_activeFilters['stockStatus'] != null && item['status'] != _activeFilters['stockStatus']) return false;
    if (_activeFilters['supplier'] != null   && item['supplier'] != _activeFilters['supplier']) return false;
    if (_activeFilters['location'] != null   && item['location'] != _activeFilters['location']) return false;
    return true;
  }).toList();

  int get _adequateCount  => _inventoryItems.where((i) => i['status'] == 'Adequate').length;
  int get _lowCount       => _inventoryItems.where((i) => i['status'] == 'Low Stock').length;
  int get _criticalCount  => _inventoryItems.where((i) => i['status'] == 'Critical').length;
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: StatefulBuilder(builder: (_, ss) => Column(mainAxisSize: MainAxisSize.min, children: [
          Row(children: [
            Expanded(child: RadioListTile<String>(title: const Text('Add'), value: 'add',
                groupValue: op, onChanged: (v) => ss(() => op = v!), activeColor: _purple,
                contentPadding: EdgeInsets.zero)),
            Expanded(child: RadioListTile<String>(title: const Text('Remove'), value: 'remove',
                groupValue: op, onChanged: (v) => ss(() => op = v!), activeColor: _purple,
                contentPadding: EdgeInsets.zero)),
          ]),
    ...
    return Scaffold(
      ...
      body: SafeArea(child: Column(children: [
        ...
        Expanded(child: RefreshIndicator(
          onRefresh: _handleRefresh, color: _purple,
          child: ListView(padding: const EdgeInsets.fromLTRB(12, 6, 12, 80), children: [
            // ── Charts row ──
            _buildChartsSection(),
            const SizedBox(height: 12),
            // ── Grid ──
            if (_filteredItems.isEmpty)
              SizedBox(height: 300, child: Center(child: Column(
            ...
            if (_filteredItems.isNotEmpty)
              GridView.builder(
                ...
                itemBuilder: (_, i) => _InventoryCard(
                  ...
                  onAdjustStock: () => _showAdjustStockDialog(_filteredItems[i]),
                  onReorder: () => Navigator.pushNamed(context, '/marketplace-product-catalog'),
                ),
              ),
          ]),
        )),
  ...
  Widget _buildHeader() => Container(
    ...
    child: Row(children: [
      const Text('Stock Inventory',
          style: TextStyle(color: _dark, fontSize: 17, fontWeight: FontWeight.w700)),
      ...
      Expanded(child: Container(
        ...
        child: Row(children: [
          ...
          GestureDetector(onTap: _showBarcodeScanner, child: const Padding(
            padding: EdgeInsets.symmetric(horizontal: 6),
            child: Icon(Icons.qr_code_scanner, color: Color(0xFF9E9E9E), size: 18))),
          GestureDetector(onTap: _showBarcodeScanner, child: const Padding(
            padding: EdgeInsets.only(right: 8),
            child: Icon(Icons.mic_none_rounded, color: Color(0xFF9E9E9E), size: 18))),
        ]),
      )),
      ...
      GestureDetector(
        ...
        child: Container(width: 38, height: 38,
          decoration: BoxDecoration(color: _bg, borderRadius: BorderRadius.circular(10),
              border: Border.all(color: const Color(0xFFEEEEEE))),
          child: const Icon(Icons.tune_rounded, color: Color(0xFF9E9E9E), size: 18)),
      ),
    ]),
  );
    ...
    child: Row(children: [
      ...
      GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            color: const Color(0xFFF0EEFA), borderRadius: BorderRadius.circular(8),
            border: Border.all(color: const Color(0xFFE5E0F5))),
          child: const Row(mainAxisSize: MainAxisSize.min, children: [
            Icon(Icons.add_rounded, color: _purple, size: 15),
            SizedBox(width: 4),
            Text('Add Item', style: TextStyle(color: _purple, fontSize: 12, fontWeight: FontWeight.w600)),
          ]),
        ),
      ),
      const SizedBox(width: 8),
      GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: [BoxShadow(color: _purple.withOpacity(0.3), blurRadius: 8, offset: const Offset(0, 2))]),
          child: const Row(mainAxisSize: MainAxisSize.min, children: [
            Icon(Icons.shopping_cart_rounded, color: Colors.white, size: 15),
            SizedBox(width: 5),
            Text('Buy', style: TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w700)),
          ]),
        ),
      ),
    ]),
    ...
    return Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Expanded(child: Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Stock Health', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: _dark)),
          const SizedBox(height: 14),
          Center(child: SizedBox(
            width: 130, height: 130,
            child: CustomPaint(
              painter: _DonutChartPainter(
                adequate: _adequateCount, low: _lowCount, critical: _criticalCount, total: _total),
              child: Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
                Text('${((_adequateCount / _total) * 100).toInt()}%',
                    style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w800, color: _dark)),
                const Text('of items', style: TextStyle(fontSize: 10, color: Color(0xFF9E9E9E))),
              ])),
            ),
          )),
          const SizedBox(height: 14),
  ...
  Widget _legendRow(Color color, String label, int count) {
    return Row(children: [
      ...
      const Spacer(),
      Text('$count', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: _dark)),
    ]);
  }
...
class _DonutChartPainter extends CustomPainter {
  ...
  const _DonutChartPainter({
    required this.adequate, required this.low,
    required this.critical, required this.total});
...
class _LineChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // ── Gradient fill ──
    final path = Path()
      ..moveTo(points.first.dx, size.height - bottomPad);
    ...
    for (final pt in points) {
      ...
      canvas.drawCircle(pt, 5.0, Paint()
        ..color = purple
        ..style = PaintingStyle.stroke
        ..strokeWidth = 1.8);
      canvas.drawCircle(pt, 2.5, Paint()..color = purple..style = PaintingStyle.fill);
    }
...
class _InventoryCard extends StatelessWidget {
  ...
  const _InventoryCard({required this.item, required this.onTap,
      required this.onAdjustStock, required this.onReorder});

  Color get _statusColor {
    switch (item['status']) {
      case 'Adequate':  return const Color(0xFF10B981);
      case 'Low Stock': return const Color(0xFFF59E0B);
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          ClipRRect(
            ...
            child: AspectRatio(
              ...
              child: Image.network(item['image'] as String, fit: BoxFit.cover,
                errorBuilder: (_, __, ___) => Container(color: const Color(0xFFF5F5F7),
          ...
          Expanded(child: Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start,
              ...
              Row(children: [
                const Text('Stock: ', style: TextStyle(fontSize: 10, color: Color(0xFF9E9E9E))),
              ...
              SizedBox(width: double.infinity, child: GestureDetector(
                ...
                child: Container(
                  ...
                  child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                    Icon(Icons.shopping_cart_outlined, color: Color(0xFF6B46C1), size: 13),
                    SizedBox(width: 5),
                    Text('Quick Reorder',
                        style: TextStyle(color: Color(0xFF6B46C1), fontSize: 11, fontWeight: FontWeight.w600)),
                  ]),
                ),
              )),
            ]),
...
class _AddItemDialogState extends State<_AddItemDialog> {
    ...
    return Dialog(
      ...
      child: Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const SizedBox(height: 12),
          _dropdown<String>('Category', _category,
              ['Raw Materials', 'Store Items', 'Finished Goods'],
              (v) => setState(() => _category = v!)),
          const SizedBox(height: 12),
          Row(children: [
            Expanded(child: _field(_stockCtrl, 'Current Stock', isNumber: true)),
            const SizedBox(width: 10),
            SizedBox(width: 100, child: _dropdown<String>('Unit', _unit,
  ...
  @override
  void dispose() { _nameCtrl.dispose(); _stockCtrl.dispose(); _minCtrl.dispose(); super.dispose(); }
}
...
class _BuyItemsModalState extends State<_BuyItemsModal> {
  ...
  final List<Map<String, dynamic>> _items = [
    {'name': 'Chicken Breast', 'status': 'Critical',  'stock': '5 kg',  'min': '15 kg',
     'image': 'https://images.unsplash.com/photo-1604503468506-a8da13d82791?w=200&q=80'},
    {'name': 'Coffee Beans',   'status': 'Critical',  'stock': '3 kg',  'min': '10 kg',
     'image': 'https://images.unsplash.com/photo-1559056199-641a0ac8b55e?w=200&q=80'},
    {'name': 'Onions',         'status': 'Low Stock', 'stock': '15 kg', 'min': '25 kg',
     'image': 'https://images.unsplash.com/photo-1587049352846-4a222e784d38?w=200&q=80'},
    {'name': 'Pasta',          'status': 'Low Stock', 'stock': '8 pk',  'min': '15 pk',
     'image': 'https://images.unsplash.com/photo-1551462147-ff29053bfc14?w=200&q=80'},
    {'name': 'Pizza Dough',    'status': 'Low Stock', 'stock': '10 pc', 'min': '20 pc',
  ...
  Color _statusColor(String s) =>
      s == 'Critical' ? _red : s == 'Low Stock' ? _amber : const Color(0xFF10B981);
      ...
      child: Column(children: [
        ...
        Padding(
          ...
          child: Row(children: [
            const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('Buy Items', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w700)),
              SizedBox(height: 3),
              Text('Select items to reorder from marketplace',
                  style: TextStyle(fontSize: 12, color: Color(0xFF9E9E9E))),
            ])),
            IconButton(icon: const Icon(Icons.close_rounded, color: Color(0xFF9E9E9E)),
                onPressed: () => Navigator.pop(context)),
          ]),
        ),
        ...
        Padding(
          ...
          child: Row(children: [
            ...
            GestureDetector(
              onTap: () => setState(() {
                _selected.length == _items.length ? _selected.clear()
                    : _selected.addAll(List.generate(_items.length, (i) => i));
              }),
              child: Text(_selected.length == _items.length ? 'Deselect All' : 'Select All',
            ...
            return GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(
                    color: sel ? _purple.withOpacity(0.4) : Colors.grey.shade200,
                    width: sel ? 1.5 : 1)),
                child: Row(children: [
                  AnimatedContainer(
                    duration: const Duration(milliseconds: 150),
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(color: sel ? _purple : Colors.grey.shade300, width: 1.5)),
                    child: sel ? const Icon(Icons.check_rounded, color: Colors.white, size: 13) : null,
                  ),
                  const SizedBox(width: 12),
          ...
          child: Column(children: [
            if (_selected.isNotEmpty)
              Padding(padding: const EdgeInsets.only(bottom: 10),
                child: Text('${_selected.length} item${_selected.length > 1 ? 's' : ''} selected',
            ...
            SizedBox(width: double.infinity, child: ElevatedButton.icon(
              ...
              label: Text(
                _selected.isEmpty ? 'Select items to continue'
    ...
    return Container(
      ...
      child: Column(children: [
        ...
        Padding(padding: const EdgeInsets.fromLTRB(16, 12, 8, 0),
          child: Row(children: [
            const Text('Scan Barcode', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)),
            const Spacer(),
        ...
        Expanded(child: Padding(padding: const EdgeInsets.all(12),
          child: ClipRRect(borderRadius: BorderRadius.circular(12),
            child: MobileScanner(controller: _ctrl,
              onDetect: (capture) {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(children: [
        ...
        Padding(padding: const EdgeInsets.fromLTRB(16, 12, 8, 0),
          child: Row(children: [
            const Text('Item Details', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)),
            const Spacer(),
        ...
        Expanded(child: SingleChildScrollView(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            const SizedBox(height: 12),
            ...[
              ['Category', item['category']], ['Current Stock', '${item['currentStock']} ${item['unit']}'],
              ['Min Threshold', '${item['minThreshold']} ${item['unit']}'], ['Status', item['status']],
              ['Supplier', item['supplier']], ['Location', item['location']], ['Last Updated', item['lastUpdated']],
            ].map((r) => Padding(
              padding: const EdgeInsets.symmetric(vertical: 8),
              child: Row(children: [
                Text(r[0] as String, style: const TextStyle(fontSize: 13, color: Color(0xFF9E9E9E))),
                const Spacer(),
                Text(r[1] as String, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600)),
              ]))),
          ]),
        )),
      ]),
